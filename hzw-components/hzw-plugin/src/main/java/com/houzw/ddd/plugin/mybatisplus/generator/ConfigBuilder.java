package com.houzw.ddd.plugin.mybatisplus.generator;
        ...
        try {
            ...
            try (
                ...
                if (StringUtils.isNotBlank(config.getSuperEntityClass())) {
                    try {
                        superFields = Arrays.stream(Class.forName(config.getSuperEntityClass()).getDeclaredFields()).map(f -> f.getName()).collect(Collectors.toList());
                ...
                while (results.next()) {
                    ...
                    // =========houzw start============
                    if(null != superFields){
                        String columnName_convert = "";
                        INameConvert nameConvert = strategyConfig.getNameConvert();
                        if (null != nameConvert) {
//                            columnName_convert = nameConvert.propertyNameConvert(field); // TODO 此处不生效哦
                        } else {
                            columnName_convert = processName(columnName, config.getColumnNaming());
                        }

                        if (superFields.contains(columnName_convert))
                            continue;
                    }
                // =========houzw end============
